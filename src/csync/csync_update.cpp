...
static int _csync_detect_update(CSYNC *ctx, std::unique_ptr<csync_file_stat_t> fs) {
  ...
  if(base.isValid()) { /* there is an entry in the database */
      ...
      CSYNC_LOG(CSYNC_LOG_PRIORITY_INFO, "Database entry found, compare: %" PRId64 " <-> %" PRId64
                ...
                (uint64_t) fs->size, (uint64_t) base._fileSize, fs->remotePerm.constData(), base._remotePerm.constData(), base._serverHasIgnoredFiles );
      ...
      if (ctx->current == LOCAL_REPLICA) {
          ...
          if (isRename && !base._checksumHeader.isEmpty() && ctx->callbacks.checksum_hook
              && fs->type == CSYNC_FTW_TYPE_FILE) {
                  fs->checksumHeader = ctx->callbacks.checksum_hook(
                      ...
                      ctx->callbacks.checksum_userdata);
              if (!fs->checksumHeader.isEmpty()) {
                  CSYNC_LOG(CSYNC_LOG_PRIORITY_TRACE, "checking checksum of potential rename %s %s <-> %s", fs->path.constData(), fs->checksumHeader.constData(), base._checksumHeader.constData());
          ...
          if (base.isValid()) {                           /* tmp existing at all */
              ...
              CSYNC_LOG(CSYNC_LOG_PRIORITY_TRACE, "remote rename detected based on fileid %s --> %s", base._path.constData(), fs->path.constData());
              fs->instruction = CSYNC_INSTRUCTION_EVAL_RENAME;
              if (fs->type == CSYNC_FTW_TYPE_DIR) {
                  csync_rename_record(ctx, base._path, fs->path);
              } else {
...
static bool fill_tree_from_db(CSYNC *ctx, const char *uri)
{
    ...
    auto rowCallback = [ctx, &count, &skipbase](const OCC::SyncJournalFileRecord &rec) {
        ...
        if( rec._etag == "_invalid_") {
            CSYNC_LOG(CSYNC_LOG_PRIORITY_TRACE, "%s selective sync excluded", rec._path.constData());
            skipbase = rec._path;
            skipbase += '/';
            return;
        }
        ...
        if( !skipbase.isEmpty() && rec._path.startsWith(skipbase) ) {
            CSYNC_LOG(CSYNC_LOG_PRIORITY_TRACE, "%s selective sync excluded because the parent is", rec._path.constData());
            return;
        } else {
            skipbase.clear();
        }
    ...
    CSYNC_LOG(CSYNC_LOG_PRIORITY_DEBUG, "%" PRId64 " entries read below path %s from db.", count, uri);
